...
def generate_launch_description():
    # 获取包路径
    package_name = 'onebot_description'
    package_share_dir = get_package_share_directory(package_name)

    # Xacro 文件路径
    xacro_file = os.path.join(package_share_dir, 'urdf', 'robots', 'onebot.urdf.xacro')
    ...
    rviz_node = Node(
        ...
        name='rviz2',
        output='screen',
        arguments=['-d', PathJoinSubstitution([FindPackageShare(package_name), 'rviz', 'onebot_description.rviz'])]  # 可选：指定 RViz 配置文件
    )
